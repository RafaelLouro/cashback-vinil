...
public class VendaControllerUnitTest extends BaseUnitTest {
	...
	public void saveVendaValida() throws Exception {
		...
		mvc.perform(MockMvcRequestBuilders.post("/venda").contentType(MediaType.APPLICATION_JSON)
	...
	public void finbByIdVendaIdValido() throws Exception {
		...
		mvc.perform(MockMvcRequestBuilders.get("/venda/" + ID_TEST).contentType(MediaType.APPLICATION_JSON))
	...
	public void findByFilterValido() throws Exception {
		Mockito.when(vendaService.findByFilter(Mockito.any(VendaFilter.class)))
				.thenReturn(new PageImpl<>(Arrays.asList(dto)));

		mvc.perform(MockMvcRequestBuilders.get("/venda/").param("page", "0").param("limit", "10")
				.param("inicio", LocalDateTime.now().toString()).param("fim", LocalDateTime.now().toString())
