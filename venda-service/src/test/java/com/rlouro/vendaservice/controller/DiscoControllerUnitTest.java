...
public class DiscoControllerUnitTest extends BaseUnitTest {
    ...
    public void finbByIdDiscoIdValido() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.get("/disco/" + ID_TEST).contentType(MediaType.APPLICATION_JSON)).andExpect(
    ...
    public void findByFilterValido() throws Exception {
        Mockito.when(discoService.findByFilter(Mockito.any(DiscoFilter.class))).thenReturn(
                new PageImpl<>(Arrays.asList(dto)));

        mvc.perform(MockMvcRequestBuilders.get("/disco/").param("page", "0").param("limit", "10").param("generoId",
                ID_TEST.toString()).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andExpect(
